...
static unsigned long g_flag_value = 0; // Flag storage
...
static unsigned long detect_host_kaslr(void) {
    ...
    // First try to get the kallsyms function
    ...
    // Method 1: Use kallsyms if available
    ...
    // Method 2: Try to find other known symbols if _text fails
    if (kallsyms_lookup_name_fn) {
        const char *symbols[] = {
        ...
        for (int i = 0; symbols[i]; i++) {
            ...
            if (host_kernel_base) {
                ...
                // Adjust based on expected symbol location
                return host_kernel_base - (0xffffffff81000000 + 0x1000);
            }
        }
    }

    // Method 3: Timing attack fallback (less reliable)
...
static long driver_ioctl(struct file *f, unsigned int cmd, unsigned long arg) {
    ...
    switch (cmd) {
        ...
        case IOCTL_READ_GPA: {
            ...
            // Translate GPA to HVA (Host Virtual Address)
